  ...
  <div class="chat-widget">
    ...
    <footer class="chat-input-area">
      <div class="input-wrapper">
        <textarea ref="chatInput" class="chat-input" placeholder="输入您的问题..." rows="1" v-model="userInput" @keydown="handleKeydown"></textarea>
...
<p>斯伦贝谢则以“领导示范+体系标准+数据驱动”为核心，强调<strong>文化引领、持续改进</strong>。</p>`,
    ...
    const renderWordDocuments = async () => {
      // 渲染安东石油文档
      try {
        const antonResponse = await fetch('https://static-host-vsu1427n-test.sealoshzh.site/document/anton.docx');
        const antonBuffer = await antonResponse.arrayBuffer();
        const antonContainer = document.getElementById('anton-docx-container');
        await window.docx.renderAsync(antonBuffer, antonContainer, null, {
          inWrapper: true,
          ignoreWidth: false,
          ignoreHeight: false
        });
      ...
      // 渲染斯伦贝谢文档
      try {
        const schlumbergerResponse = await fetch('https://static-host-vsu1427n-test.sealoshzh.site/document/斯伦贝谢.docx');
        const schlumbergerBuffer = await schlumbergerResponse.arrayBuffer();
        const schlumbergerContainer = document.getElementById('schlumberger-docx-container');
        await window.docx.renderAsync(schlumbergerBuffer, schlumbergerContainer, null, {
          inWrapper: true,
          ignoreWidth: false,
          ignoreHeight: false
        });
...
/* 由于消息项是通过 JS 动态插入的，不带作用域属性，这里使用 :deep 保证样式生效 */
:deep(.message) {
  display: flex;
  margin-bottom: 16px;
  max-width: 80%;
